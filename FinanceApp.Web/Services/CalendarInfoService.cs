...
namespace FinanceApp.Web.Services
{
	public class CalendarInfoService : ICalendarInfoService
	{
		...
		public CalendarInfoService(HttpClient httpClient)
		{
		...
		public async Task<IEnumerable<EarningsCalendarDto>> GetCalendarInfo()
		{
			...
			try
			{
				var calendarInfo = await _httpClient.GetFromJsonAsync<IEnumerable<EarningsCalendarDto>>($"https://localhost:7282/GetEarningsCalendar/api/earningscalendar?from={from:yyyy-MM-dd}&to={to:yyyy-MM-dd}");
				return calendarInfo;
			}
			catch (Exception ex)
			{
